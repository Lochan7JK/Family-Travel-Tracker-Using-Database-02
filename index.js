...
let users = [
  { id: 1, name: "Lochan", color: "teal" },
  { id: 2, name: "Zendaya", color: "powderblue" },
];
...
async function checkVisisted() {
  const result = await db.query(
    ...
    [currentUserId]
    // "SELECT TRIM(country_code) AS country_code FROM visited_countries2 JOIN users ON users.id = user_id WHERE user_id = $1; ",
    // [currentUserId]
  );
  let countries = [];
...
app.post("/add", async (req, res) => {
  ...
  try {
    ...
    // console.log("Country doesn't exists");
